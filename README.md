...
`BWA_MEM2_MEM_SORT` (mapping + sort) instead uses a combined [Seqera Wave](https://seqera.io/wave/) multi-package image, `community.wave.seqera.io/library/bwa-mem2_htslib_samtools:db98f81f55b64113`, pinned by both tag and digest, verified directly with `docker run ... bwa-mem2 version` / `samtools --version`: bwa-mem2 **2.2.1** (the same real binary already running under the 2.3-tagged image above) and samtools **1.22.1**. No mulled/Wave image combining bwa-mem2 with samtools **1.24** -- the version pinned everywhere else in this pipeline (`SAMTOOLS_MERGE`, `SAMTOOLS_INDEX`, `SAMTOOLS_QC`, `SAMTOOLS_FAIDX`) -- was found to exist; this 1.22.1-vs-1.24 gap is a deliberate, documented limitation of this container choice (Issue #8), not an oversight. samtools' BAM output format is not version-specific, so this does not affect the correctness of the coordinate-sorted BAM consumed by the 1.24-pinned stages downstream. A combined image (rather than two separate containers) is required specifically because bwa-mem2 and samtools sort run *concurrently* inside one shell pipe and must share a single task's CPU/memory budget -- see below.
