...
`records_not_selected` (`raw_all_records - raw_snp_records - raw_indel_records`, in `variant_type_accounting.tsv`) is not guaranteed to be zero or positive. `GATK SelectVariants` places a record into `cohort.snp.vcf.gz` or `cohort.indel.vcf.gz` whenever *any* allele at that site matches the requested type, so a multiallelic record carrying both a SNP and an indel allele is counted in both outputs and can drive `records_not_selected` negative; `snp_indel_overlap_records` (records whose position appears in both type-specific selections) is reported alongside it as direct evidence when that happens, rather than the value being hidden or clamped. A positive value instead indicates MNP/other/multiallelic records present in `raw/all` but excluded from both type-specific selections. `raw/snp` and `raw/indel` are therefore not guaranteed to partition `raw/all`, in either direction.
