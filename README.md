...
`BWA_MEM2_MEM_SORT` (mapping + sort) instead uses a combined [Seqera Wave](https://seqera.io/wave/) multi-package image, `community.wave.seqera.io/library/bwa-mem2_htslib_samtools:db98f81f55b64113`, pinned by both tag and digest, verified directly with `docker run ... bwa-mem2 version` / `samtools --version`: bwa-mem2 **2.2.1** (the same real binary already running under the 2.3-tagged image above) and samtools **1.22.1**. No mulled/Wave image combining bwa-mem2 with samtools **1.24** -- the version pinned everywhere else in this pipeline (`SAMTOOLS_MERGE`, `SAMTOOLS_INDEX`, `SAMTOOLS_QC`, `SAMTOOLS_FAIDX`) -- was found to exist; this 1.22.1-vs-1.24 gap is a deliberate, documented limitation of this container choice (Issue #8), not an oversight. This pipeline's synthetic fixture confirms the resulting BAM carries the expected coordinate-sort header and produces byte-identical downstream variant-calling output through the 1.24-pinned stages -- but BGZF interoperability between samtools versions does not by itself guarantee identical sort tie-break ordering, compression, or bug fixes, and the SAM spec leaves same-RNAME/POS record order unspecified. Any behavioral difference between 1.22.1 and 1.24 on real data is unverified and left to Phase 5 (real-reference profiling) to surface. A combined image (rather than two separate containers) is required specifically because bwa-mem2 and samtools sort run *concurrently* inside one shell pipe and must share a single task's CPU/memory budget -- see below.
...
- **CPU split**: bwa-mem2 gets 80% of `task.cpus` (floored at 1), samtools sort gets the remaining 20% (floored at 1). bwa-mem2's alignment/scoring work dominates the pair's total CPU cost and scales close to linearly with threads; samtools sort's speedup from additional threads is smaller past a handful. This 80/20 split is a documented starting assumption, not a measurement -- Issue #8 Phase 5 (real-reference profiling) is expected to confirm or retune it once real hardware and a real reference are available.
- **Memory split**: `samtools sort -m` sets memory *per thread* (768 MiB/thread if omitted, regardless of what the task actually has available). The formula reserves 50% of `task.memory` for bwa-mem2 (its footprint scales with reference/index size, which this formula deliberately does not try to predict ahead of real measurement), a fixed 512 MiB for OS/tool overhead, and divides the remainder evenly across sort's own threads -- clamped so it can never fall below 1 MiB/thread even on a pathologically small `task.memory`.

Both formulas are implemented and documented in-line in `modules/local/bwa_mem2_mem_sort.nf`. `BWA_MEM2_MEM_SORT` and `BWA_MEM2_INDEX` each have their own resource label (`process_mapping`, `process_bwa_index`), independent of `process_high` and of each other, so they can be tuned separately once Phase 5's real-reference measurements are available.
...
- real-reference (e.g. Longxiaodou 4) BWA-MEM2 index-build and mapping+sort resource measurement (Issue #8 Phase 5): the 80/20 CPU split and the memory-split formula above are documented starting assumptions, not measurements, pending access to real reference data and representative hardware
